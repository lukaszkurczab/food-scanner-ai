...
function run(cmd, args, opts = {}) {
  const env = {
    ...process.env,
    JAVA_TOOL_OPTIONS:
      process.env.JAVA_TOOL_OPTIONS || "-Xmx6g -XX:MaxMetaspaceSize=1536m",
    GRADLE_OPTS:
      process.env.GRADLE_OPTS ||
      '-Xmx6g -Dorg.gradle.jvmargs="-Xmx6g -XX:MaxMetaspaceSize=1536m"',
...
function main() {
  ...
  if (artifacts.length === 0) {
    ...
    if (moved.length === 0) {
      if (ciMode) {
        console.warn(
          "! No artifacts found. CI mode detected, skipping artifact requirement.",
        );
      } else {
      ...
      console.log(
        "Moved artifacts:",
        moved.map((m) => path.relative(process.cwd(), m)).join(", "),
      );
    }
  } else {
    console.log(
      "Artifacts:",
      artifacts.map((m) => path.relative(process.cwd(), m)).join(", "),
    );
